...
HELPER_HEADER = pathlib.Path("__eqt/excstring.hpp")
HELPER_HEADER_INCLUDE = f"#include <{HELPER_HEADER.as_posix()}>"
...
utils:
  finder:
    any:
      ...
      - matches: find-string-tree-sitter-workaround
  ...
  find-string-tree-sitter-workaround:
    kind: string_literal
    follows:
      kind: identifier
      regex: ^return$
...
THROWING_FUNC_NAMES = (
    ...
    # calling '__throw_...(...)' internally
    # sometimes prefixed with this->
    "_M_check",
    "_M_check_len",
    "_M_check_length",
)
...
def worker(root: pathlib.Path, p: pathlib.Path, tagged_rules):
    ...
    if HELPER_HEADER_INCLUDE in data:
        return (p, None)
